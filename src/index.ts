...
2) 운세는 "오늘 당신의 운세는 '<간결한 한 줄 요약>입니다.' " 로 시작하며 중간길이 형식으로 작성합니다.
...
async function handleAuthRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === "/api/auth/register") {
      ...
      if (birthdate && !/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/.test(birthdate)) {
        return new Response("생년월일 형식이 올바르지 않습니다.", { status: 400 });
      }
    ...
    if (url.pathname === "/api/auth/login") {
      ...
      // 비밀번호 검증 (상수 시간 비교로 Timing Attack 방지)
 ...
 * 일반 !== 비교는 문자 위치마다 시간이 달라 서명 추측에 악용될 수 있음
 */
async function timingSafeEqual(a: string, b: string): Promise<boolean> {
  const encoder = new TextEncoder();
  // 임시 키로 HMAC을 계산해 고정 길이로 정규화한 뒤 XOR 비교
  ...
  const [aHmac, bHmac] = await Promise.all([
    crypto.subtle.sign("HMAC", secretKey, encoder.encode(a)),
    crypto.subtle.sign("HMAC", secretKey, encoder.encode(b)),
  ]);
  const aArr = new Uint8Array(aHmac);
  const bArr = new Uint8Array(bHmac);
  let diff = 0;
...
async function handleChatRequest(
  ...
  try {
    ...
    // 허용된 역할(user/assistant)만 통과, 최근 20개로 제한 (Prompt Injection 방어)
    const allowedRoles = new Set(["user", "assistant"]);
    const nonSystem = messages
      .filter((msg) => allowedRoles.has(msg.role) && typeof msg.content === "string")
      .slice(-20);
    ...
    // DB 저장 전 sanitize 적용
    const userLastMessage = sanitize(userMessages.slice(-1)[0]?.content || "");

    // JWT payload의 생년월일 사용 (클라이언트 [생년월일] 태그 무시 → 조작 불가)
    let zodiacInfo = "";
    ...
    // [생년월일] 태그 메시지 제거 (서버 birthdate를 사용하므로 불필요)
    const filteredMessages = nonSystem.filter(m => !m.content.startsWith("[생년월일]"));
